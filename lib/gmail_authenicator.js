...
function createApplication() {
  ...
  /**
   * Stop Watch Email Topic Notification
   */
  app.stopWatch = function (auth, opts) {
    console.log("Authorized Stop Subscription Web Request");
    ...
    gmail.users.stop(options, function (err, res) {
      if (err) {
        console.log("Error in stop subscription", err);
        // doSomething here;
        opts.res.end(`Error in stop subscription ${err}`)
        return;
      }

      console.log("Success stop to subscription");

      let response = { code : "000" , message : "Stop subscription is completed"};

      // Response to client
      opts.res.end(JSON.stringify(response));
    });
  }
